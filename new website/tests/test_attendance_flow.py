...
class AttendanceFlowTests(unittest.TestCase):
    def setUp(self):
        app_module.SUPABASE_ENABLED = False
        ...
        app_module.write_employees([
            {'employee_id': '101', 'name': 'John Smith', 'department': 'Recruiting', 'office': 'Head Office', 'password': 'pass101', 'username': 'john', 'shift_start': '09:00', 'shift_end': '18:00', 'allowed_ips': ''},
            {'employee_id': '102', 'name': 'Aisha Khan', 'department': 'Sales', 'office': 'Branch Office', 'password': 'pass102', 'username': 'aisha', 'shift_start': '09:00', 'shift_end': '18:00', 'allowed_ips': ''},
            {'employee_id': '103', 'name': 'Ravi Patel', 'department': 'Operations', 'office': 'South Office', 'password': 'pass103', 'username': 'ravi', 'shift_start': '09:00', 'shift_end': '18:00', 'allowed_ips': ''},
        ])
    ...
    def test_failed_employee_login_is_audited(self):
        app_module.write_csv(app_module.LOGIN_LOGS_CSV, app_module.LOGIN_LOG_HEADERS, [])

        response = self.client.post('/login', data={
            'login_type': 'employee',
            'employee_id': '101',
            'employee_password': 'wrong-password',
        }, environ_overrides={'REMOTE_ADDR': '192.168.0.10'})
        ...
        self.assertEqual(login_logs[0]['user_id'], '101')
        self.assertEqual(login_logs[0]['role'], 'employee')
        self.assertEqual(login_logs[0]['status'], 'failed-invalid-credentials')

    def test_admin_dashboard_shows_system_wide_login_and_denied_logs(self):
        app_module.write_csv(app_module.LOGIN_LOGS_CSV, app_module.LOGIN_LOG_HEADERS, [
            {'timestamp': '2026-07-16 09:00:00.000001', 'user_id': '101', 'role': 'employee', 'detected_ip': '192.168.0.10', 'status': 'success'},
            {'timestamp': '2026-07-16 09:05:00.000001', 'user_id': '102', 'role': 'employee', 'detected_ip': '192.168.0.11', 'status': 'failed-invalid-credentials'},
            {'timestamp': '2026-07-16 09:10:00.000001', 'user_id': 'admin', 'role': 'admin', 'detected_ip': '192.168.0.12', 'status': 'success'},
        ])
        app_module.write_csv(app_module.DENIED_CSV, app_module.DENIED_HEADERS, [
            {'timestamp': '2026-07-16 09:15:00.000001', 'employee_id': '101', 'action': 'login', 'detected_ip': '8.8.8.8', 'reason': 'IP not in allowed list'},
            {'timestamp': '2026-07-16 09:20:00.000001', 'employee_id': 'admin', 'action': 'dashboard', 'detected_ip': '9.9.9.9', 'reason': 'IP not in allowed list'},
        ])
        with self.client.session_transaction() as sess:
            sess.update({'role': 'admin', 'admin_id': 'admin'})

        response = self.client.get('/admin', environ_overrides={'REMOTE_ADDR': '192.168.0.10'})
        html = response.get_data(as_text=True)

        self.assertEqual(response.status_code, 200)
        self.assertIn('101', html)
        self.assertIn('102', html)
        self.assertIn('failed-invalid-credentials', html)
        self.assertIn('8.8.8.8', html)
        self.assertIn('9.9.9.9', html)

    def test_employee_cannot_view_admin_system_logs(self):
        with self.client.session_transaction() as sess:
            sess.update({
                'role': 'employee',
                'employee_id': '101',
        ...
        response = self.client.get('/admin', environ_overrides={'REMOTE_ADDR': '192.168.0.10'}, follow_redirects=False)
    ...
    def test_clock_in_uses_employee_shift_for_late_minutes(self):
        employees = app_module.read_employees()
        employees[0]['shift_start'] = '11:00'
        employees[0]['shift_end'] = '20:00'
        app_module.write_employees(employees)
        app_module.write_csv(app_module.ATTENDANCE_CSV, app_module.ATT_HEADERS, [])
        app_module.write_csv(app_module.LOGIN_LOGS_CSV, app_module.LOGIN_LOG_HEADERS, [])
        with self.client.session_transaction() as sess:
            sess.update({
        ...
        with patch('app.app_now', return_value=app_module.datetime(2026, 7, 20, 11, 15, 0)):
            response = self.client.post('/clock-in', environ_overrides={'REMOTE_ADDR': '192.168.0.10'})

        self.assertTrue(response.get_json()['success'])
        record = app_module.read_attendance_records()[0]
        self.assertEqual(record['clock_in_time'], '11:15:00')
        self.assertEqual(record['late_minutes'], '15')

    def test_employee_allowed_ips_restrict_login(self):
        employees = app_module.read_employees()
        employees[0]['allowed_ips'] = '192.168.0.99'
        app_module.write_employees(employees)
        app_module.write_csv(app_module.DENIED_CSV, app_module.DENIED_HEADERS, [])
        ...
        response = self.client.post('/login', data={
            ...
            'employee_id': '101',
            'employee_password': 'pass101',
        }, environ_overrides={'REMOTE_ADDR': '192.168.0.10'}, follow_redirects=True)

        self.assertEqual(response.status_code, 403)
        self.assertIn('current network is not authorized', response.get_data(as_text=True))

    def test_activity_log_records_clock_and_device_metadata(self):
        app_module.write_csv(app_module.ATTENDANCE_CSV, app_module.ATT_HEADERS, [])
        app_module.write_csv(app_module.LOGIN_LOGS_CSV, app_module.LOGIN_LOG_HEADERS, [])
        with self.client.session_transaction() as sess:
            sess.update({
        ...
        response = self.client.post('/clock-in', environ_overrides={
            'REMOTE_ADDR': '192.168.0.10',
            'HTTP_USER_AGENT': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0',
        })

        self.assertTrue(response.get_json()['success'])
        log = app_module.read_csv(app_module.LOGIN_LOGS_CSV)[0]
        self.assertEqual(log['employee_name'], 'John Smith')
        self.assertEqual(log['employee_id'], '101')
        self.assertEqual(log['activity_type'], 'Clock In')
