...
BREAK_HEADERS = ['break_id','attendance_id','employee_id','date','break_start','break_end','duration_seconds','duration_display']
    ...
    try:
        ...
        if not ip_ok(ip):
            with open(get_storage_path(DENIED_CSV), 'a', newline='', encoding='utf-8') as f:
                csv.writer(f).writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), session['employee_id'], 'clock-in', ip, 'IP not in allowed list'])
