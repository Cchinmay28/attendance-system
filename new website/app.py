...
EMP_HEADERS = ['employee_id','name','department','office','password','username','shift_start','shift_end','allowed_ips']
...
if not read_csv(EMPLOYEES_CSV):
    with open(get_storage_path(EMPLOYEES_CSV), 'a', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['101', 'John Smith', 'Recruiting', 'Head Office', 'pass101', '101', DEFAULT_SHIFT_START, DEFAULT_SHIFT_END, ''])
        w.writerow(['102', 'Aisha Khan', 'Sales', 'Branch Office', 'pass102', '102', DEFAULT_SHIFT_START, DEFAULT_SHIFT_END, ''])
        w.writerow(['103', 'Ravi Patel', 'Operations', 'South Office', 'pass103', '103', DEFAULT_SHIFT_START, DEFAULT_SHIFT_END, ''])
else:
    emps = read_employees()
...
def admin():
    ...
    return render_template('admin.html',
        ...
        login_logs=sorted(login_logs, key=lambda x: x.get('timestamp', '')),
...
def employee_form_payload(existing_id=None):
    ...
    allowed_ips = ','.join(split_allowed_ips(request.form.get('allowed_ips', '')))
...
def redirect_admin_with_message(message, ok=True):
    return redirect(url_for('admin', employee_message=message, employee_status='ok' if ok else 'error') + '#employees')
...
def api_time():
    ...
    return jsonify(
        ...
        offset_seconds=int((now - datetime.utcnow().replace(tzinfo=now.tzinfo)).total_seconds()) if now.tzinfo else 0,
    )
